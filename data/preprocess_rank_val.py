...
def preprocess(args):
    ...
    qrels_val = load_rel(args.data_dir + "qrels_val.csv")
    ...
    query_val_df = pd.read_csv(args.data_dir + 'query_val.csv', sep=',', encoding='utf8')

    for index, row in tqdm(query_val_df.iterrows(), desc="Processing queries"):
        Item = dict()
        ...
        for negative_id in ir_tree_rank[str(query_id)]:
            if negative_id not in qrels_val[query_id]:
        ...
        for positive_id in qrels_val[query_id]:
    ...
    with open(args.output_data_dir + 'rank_val.json', 'w') as f2:
        f2.write(jsonArr)
